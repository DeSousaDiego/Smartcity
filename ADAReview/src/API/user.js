...
const sendFormRegister = async (formData) => {
  return await axios.post(`${userURL}/register`, formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
      'Accept-Version': versionNumber
    }
...
const getActiveUser = async (token) => {
  return await axios.get(`${userURL}/active/active`, {
    headers: {
      'Authorization': 'Bearer ' + token,
      'Accept-Version': versionNumber
    }
...
export {sendForm, sendFormRegister, getAllUsers, getActiveUser, getUserById, deleteUser, updateUser, login, logout, checkToken};
