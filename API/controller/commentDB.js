...
module.exports.updateComment = async (req, res) => {
    const id = parseInt(req.params.id); //attention ! Il s'agit de texte !
...
module.exports.addLike = async (req, res) => {
    ...
    try{
        if(isNaN(id)){
            ...
            await commentModele.addLike(client, id);
            res.sendStatus(204)
        }
    } catch (error){
        console.error(error);
...
module.exports.removeLike = async (req, res) => {
    ...
    try{
        if(isNaN(id)){
            ...
            await commentModele.removeLike(client, id);
            res.sendStatus(204)
        }
    } catch (error){
        console.error(error);
...
module.exports.addDislike = async (req, res) => {
    ...
    try{
        if(isNaN(id)){
            ...
            await commentModele.addDislike(client, id);
            res.sendStatus(204)
        }
    } catch (error){
        console.error(error);
...
module.exports.removeDislike = async (req, res) => {
    const idTexte = req.params.id; //attention ! Il s'agit de texte !
    const id = parseInt(idTexte);
    const client = await pool.connect();
    ...
    try{
        if(isNaN(id)){
            ...
            await commentModele.removeDislike(client, id);
            res.sendStatus(204)
        }
    } catch (error){
        console.error(error);
