...
module.exports.updateUser = async (req, res) => {
    if(req.session){
        ...
        if(userObj.id !== parseInt(toUpdate.id) && userObj.authLevel !== "admin"){
            res.sendStatus(401);
        }
        else{
...
module.exports.getUser = async (req, res) => {
    if(req.session){
        ...
        const client = await pool.connect();
        try{
            const {rows: users} = await UserModel.getUserById(client, userObj.id);
            const user = users[0];
            if(user !== undefined){
                res.json(user);
            } else {
...
module.exports.createUser = async (req, res) => {
    ...
    if(!imageError){
        ...
        if (userObj === undefined  || userObj.role !== "admin") {
            newData.role = "user";
        }
        try{
...
module.exports.deleteUser = async (req, res) => {
    if(req.session){
        ...
        if(userObj.id !== parseInt(id) && userObj.authLevel !== "admin"){
            res.sendStatus(401);
        }
        else{
            try{
