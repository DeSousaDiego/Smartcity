...
module.exports.updateUser = async (client, id, username, emailAddress, password, role, country, phoneNumber, newsLetter, image) => {
    ...
    if(image !== undefined){
        params.push(image);
        querySet.push(` profile_picture_path = $${params.length} `);
    }
    if(params.length > 0){
        query += querySet.join(',');
        params.push(id);
