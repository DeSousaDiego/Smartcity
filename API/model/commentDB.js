...
module.exports.getAllCommentsFromReviewId = async (client, idReview) =>{
    return await client.query(`
        SELECT c.id, a.username, r.title , c.content, c.likes_counter, c.dislikes_counter, a.profile_picture_path AS img_path, a.id AS user_id, r.id AS review_id
        FROM comment c
...
module.exports.addLike = async (client, id) => {
    return await client.query("UPDATE comment SET likes_counter = likes_counter + 1 WHERE id = $1", [id]);
};

module.exports.removeLike = async (client, id) => {
    return await client.query("UPDATE comment SET likes_counter = likes_counter - 1 WHERE id = $1", [id]);
};

module.exports.addDislike = async (client, id) => {
    return await client.query("UPDATE comment SET dislikes_counter = dislikes_counter + 1 WHERE id = $1", [id]);
};

module.exports.removeDislike = async (client, id) => {
    return await client.query("UPDATE comment SET dislikes_counter = dislikes_counter - 1 WHERE id = $1", [id]);
};